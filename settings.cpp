...
namespace settings {
    ...
    void load() {
        ...
        if (String(data.ap.ssid) == "pwned") {
            strncpy(data.ap.ssid, "CRABOX", 32);
            data.ap.ssid[32] = '\0';
            changed = true;
            save();
        }
        if (String(data.ap.password) == "deauther") {
            strncpy(data.ap.password, "craboxadmin", 64);
            data.ap.password[64] = '\0';
            changed = true;
            save();
        }
